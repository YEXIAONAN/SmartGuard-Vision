...
const createDefaultState = () => ({
  ...
  eventDistribution: {
    labels: [],
    values: [],
    colors: ['#4c8bf5', '#7aa9ff', '#f3b24d', '#f08c6a', '#f08c6a', '#d96e6e'],
  },
  riskLevel: [],
  areaRisk: [],
  alertTrend: {
...
const adaptDashboardData = ({ health, overview }) => ({
  project: {
  ...
  eventDistribution: {
    ...
    colors: ['#4c8bf5', '#7aa9ff', '#f3b24d', '#f08c6a', '#d96e6e', '#8a8ff0'],
  },
  ...
  areaRisk: overview.area_risk_ranking.map((item, index) => ({
    name: item.name,
    value: item.count,
    color: ['#d85c5c', '#eb8b56', '#f3b24d', '#72a6ff', '#4c8bf5', '#7f95ff'][index % 6],
  })),
  alertTrend: {
    labels: overview.seven_day_alert_trend.map((item) => item.date),
